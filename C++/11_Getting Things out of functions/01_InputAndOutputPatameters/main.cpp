#include <iostream>
#include <string>

void max_str(const std::string &input1, const std::string &input2, std::string &output)
// WE are passing output as reference because we want change in the value
{
    if (input1 > input2)
    {
        output = input1;
    }
    else
    {
        output = input2;
    }
}

void max_int(int input1, int input2, int &output)
{
    if (input1 > input2)
    {
        output = input1;
    }
    else
    {
        output = input2;
    }
} // Here we are not passing by reference because int is already small so it is faster to copy then passing it by reference

void max_double(double input1, double input2, double *output)
{
    if (input1 > input2)
    {
        *output = input1;
    }
    else
    {
        *output = input2;
    }
}

int main()
{
    /*
    std::string out_str;
    std::string string1("Alabama");
    std::string string2("Bellevue");

    max_str(string1, string2, out_str);
    std::cout << "max_str :" << out_str << std::endl;
*/

    /*
      int out_int{};
       int in1{24};
       int in2{45};
       max_int(in1, in2, out_int);
       std::cout << "max_int :" << out_int << std::endl;
       */

    double out_double{};
    double in_double1{45.5};
    double in_double2{45.6};
    max_double(in_double1, in_double2, &out_double);
    std::cout << "max_double :" << out_double << std::endl;
    return 0;
}